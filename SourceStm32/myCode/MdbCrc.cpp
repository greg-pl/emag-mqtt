...
uint16_t MdbCrc::Proceed(uint16_t Crc, uint8_t inp) {
	return (CrcTab[(Crc ^ inp) & 0xff] ^ (Crc >> 8));
}

uint16_t MdbCrc::Build(const uint8_t *p, int cnt) {
	uint16_t crc;
...
bool MdbCrc::Check(const uint8_t *p, int cnt) {
	return (Build(p, cnt) == 0);
}

void MdbCrc::Set(uint8_t *p, int cnt) {
	uint16_t crc;
