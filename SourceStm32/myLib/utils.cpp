...
bool Token::get(const char **inp, char *tok, int size, char *pSep) {
	return get(inp, ",; =", tok, size, pSep);
}

bool Token::get(const char **inp, char *tok, int size) {
	return get(inp, tok, size, NULL);
}

bool Token::get(const char **inp) {
	char buf[4];
	return get(inp, buf, sizeof(buf));
}

bool Token::getAsInt(const char **inp, int *val) {
	char tok[20];
	bool q = get(inp, tok, sizeof(tok));
	if (q) {
		*val = atoi(tok);
	}
	return q;
...
bool Token::getAsInt(const char **inp, const char *sepStr, int *val) {
	char tok[20];
	char msep;
	bool q = get(inp, sepStr, tok, sizeof(tok), &msep);
	if (q) {
		*val = atoi(tok);
	}
	return q;
